...
type MarketStatus = "Aberto" | "Fechado";
type Market = {
  id: string;
  name: string;
  status: MarketStatus;
...
const markets: Market[] = [
  { id: "central", name: "Mercado Central", status: "Aberto", address: "Av. Brasil, 1240 · Centro", phone: "(11) 3456-7800", manager: "Carlos Mendes", revenue: 82450, sales: 742, replenishments: 7, stockAlerts: 4, expiryAlerts: 3, inconsistencies: 2, updatedAt: "18:17" },
  { id: "jardim", name: "Mercado Jardim", status: "Aberto", address: "Rua das Flores, 455 · Jardim Sul", phone: "(11) 3456-7810", manager: "Fernanda Lima", revenue: 61780, sales: 583, replenishments: 4, stockAlerts: 2, expiryAlerts: 5, inconsistencies: 1, updatedAt: "18:15" },
  { id: "avenida", name: "Mercado Avenida", status: "Fechado", address: "Av. das Nações, 890 · Bela Vista", phone: "(11) 3456-7820", manager: "Rafael Souza", revenue: 40060, sales: 391, replenishments: 3, stockAlerts: 3, expiryAlerts: 1, inconsistencies: 1, updatedAt: "17:58" },
];
...
export function OwnerDashboard({ onLogout }: { onLogout: () => void }) {
  ...
  const totals = useMemo(() => {
    ...
    }), { revenue: 0, sales: 0, replenishments: 0, stockAlerts: 0, expiryAlerts: 0, inconsistencies: 0, open: 0 });
  }, [selectedMarket]);
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <div className={`transition-[padding] duration-300 ${collapsed ? "lg:pl-[76px]" : "lg:pl-[270px]"}`}>
        ...
        {detailMarket ? <MarketDetail market={detailMarket} onBack={() => { setDetailMarket(null); setActive("Visão geral"); }} /> : (
          <DashboardOverview active={active} totals={totals} visibleMarkets={visibleMarkets} query={query} setQuery={setQuery} openMarket={openMarket} onAdd={() => setToast("Cadastro de novo mercado iniciado em modo demonstrativo.")} period={period} />
        )}
      </div>
    </main>
  );
...
function DashboardOverview({ active, totals, visibleMarkets, query, setQuery, openMarket, onAdd, period }: { active: string; totals: ReturnType<typeof emptyTotals>; visibleMarkets: Market[]; query: string; setQuery: (value: string) => void; openMarket: (market: Market) => void; onAdd: () => void; period: string }) {
  ...
  const metrics = [
    ...
    { title: "Mercados em funcionamento", value: `${totals.open}/${selectedCount(totals)}`, note: "Operando neste momento", icon: Building2, tone: "positive" },
  ...
  return <section className="mx-auto max-w-[1680px] p-4 sm:p-6 lg:p-8">
    ...
    <div className="mt-6 grid gap-5 xl:grid-cols-[minmax(0,1.55fr)_minmax(340px,.85fr)]">
      ...
      <ChartCard title="Comparação entre mercados" subtitle="Participação no faturamento">
        <div className="mt-6 space-y-5">{markets.map((market, index) => <HorizontalBar key={market.id} label={market.name.replace("Mercado ", "")} value={market.revenue} max={82450} detail={money(market.revenue)} muted={index === 2} />)}</div>
    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-3">
      <RankCard title="Produtos mais vendidos" subtitle="Unidades hoje" icon={BarChart3} items={[{ name: "Leite integral 1L", value: "486 un.", width: 100 }, { name: "Arroz tipo 1 5kg", value: "352 un.", width: 72 }, { name: "Café tradicional 500g", value: "297 un.", width: 61 }]} />
      <RankCard title="Produtos com menor giro" subtitle="Últimos 30 dias" icon={PackageMinus} items={[{ name: "Azeitona premium 200g", value: "3 un.", width: 18 }, { name: "Molho especial 350ml", value: "5 un.", width: 28 }, { name: "Chá importado 20un", value: "7 un.", width: 38 }]} muted />
...
function emptyTotals() { return { revenue: 0, sales: 0, replenishments: 0, stockAlerts: 0, expiryAlerts: 0, inconsistencies: 0, open: 0 }; }
function selectedCount(totals: ReturnType<typeof emptyTotals>) { return totals.revenue === markets.reduce((sum, market) => sum + market.revenue, 0) ? markets.length : 1; }
