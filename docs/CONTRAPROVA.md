...
Lista de verificação de **todos** os itens da _Documentação Funcional Completa_ (v1.0, 23/09/2026) e dos problemas encontrados na análise.
...
| RF-ORG-03 | Mostrar no card nome, endereço, telefone, gerente, faturamento do dia e alertas.  |          | Parcial | Nome/endereço/telefone/gerente reais (PR B1.5 · `getManagerNamesByMarket` em `src/lib/markets-api.ts`); faturamento e alertas ficam em 0 até estoque/PDV existirem (B2–B5) — números fictícios seriam mentira, não uma pendência |
...
| ID        | Item                                                                                                                                                                             | Decisão? | Status   | Evidência |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | -------- | --------- |
| RF-ORG-05 | Permitir convidar gerente e funcionários por mercado.                                                                                                                            |          | Feito    | PR B1.5 · `public.create_invite` + tela "Equipe e acessos" (`src/components/owner-sections.tsx`, `src/lib/invites-api.ts`), testado em `supabase/tests/50_invites_test.sql`         |
| RN-ORG-04 | Funcionário só recebe permissões após aceitar convite e ser vinculado a mercado.                                                                                                 |          | Feito    | PR B1.5 · `public.accept_invite` só cria `company_members`/`member_markets` no aceite; convite vive em tabela própria até lá                                                        |
...
| G-06      | Lacuna: limites do gerente ao convidar/editar equipe (não pode dar mais acesso do que tem)                                                                                       |          | Feito    | PR B1.5 · `create_invite` restringe o gerente a convidar conferente/repositor, exige mercado(s) e confere `private.can_access_market` para cada um antes de gravar                  |
| F-5.2     | Fluxo: convite e entrada de funcionários                                                                                                                                         |          | Feito    | PR B1.5 · convite (e-mail/papel/mercados) → aceite pelo token (`/convite`) → login ou criação de conta → vínculo criado; reenviar renova 7 dias (DEC-B1-06), revogar impede o aceite |
| AUD-02    | Evento de auditoria: Criação/edição/inativação/vínculo de empresa, mercado e usuário (seção 8.1)                                                                                 |          | Feito    | PR B1.5 · `audit_invites` (trigger genérico) registra criação/aceite/revogação/reenvio de convite                                                                                   |
...
| RN-EST-01     | Saldo = saldo anterior + entradas + transferências recebidas + ajustes positivos - saídas - transferências enviadas - vendas - perdas - devoluções ao fornecedor + ajustes negativos/positivos conforme sinal.                                    |          | Feito    | PR B3.2 · implementado como RN-CRT-EST-01 (ver E-02: o texto desta regra soma ajustes duas vezes, a versão correta é a de baixo) |
| RN-EST-02     | O saldo deve existir por mercado, endereço, produto e lote quando aplicável.                                                                                                                                                                      |          | Parcial  | PR B3.2 · por mercado/endereço/produto pronto; por lote depende do B3.3 |
| RN-EST-03     | Movimento confirmado é imutável; correção ocorre por estorno ou movimento compensatório.                                                                                                                                                          |          | Feito    | PR B3.2 · testado (ledger sem UPDATE/DELETE, estorno cria linha compensatória) |
| RN-EST-04     | Permitir ou bloquear estoque negativo está [A DEFINIR].                                                                                                                                                                                           | Sim      | Feito    | PR B3.2 · decisão em DECISOES.md; `register_stock_movement` bloqueia saldo negativo sem justificativa, testado |
| RN-EST-08     | Toda inconsistência deve guardar valores esperados, contados e diferença.                                                                                                                                                                         |          | Pendente | É sobre contagem/inventário — depende do B3.6, ainda não construído |
| RN-CRT-EST-01 | Saldo por endereço = saldo anterior + entradas confirmadas + transferências recebidas + ajustes positivos - saídas confirmadas - transferências enviadas - vendas atribuídas ao endereço - perdas - devoluções ao fornecedor - ajustes negativos. |          | Feito    | PR B3.2 · view `stock_balances` (soma das quantidades já assinadas), testado com o exemplo adaptado da seção 4.1 |
| RN-CRT-EST-02 | Saldo total do mercado = soma dos saldos de todos os endereços ativos, incluindo áreas de recebimento, depósito, gôndola e trânsito conforme política [A DEFINIR].                                                                                | Sim      | Feito    | PR B3.2 · decisão em DECISOES.md: soma dos endereços de depósito por enquanto; gôndola/trânsito entram com B5/B3.5 |
| RN-CRT-EST-03 | Saldo disponível = saldo físico/teórico - reservado - bloqueado. Reserva e estoque em trânsito estão [A DEFINIR].                                                                                                                                 | Sim      | Feito    | PR B3.2 · decisão em DECISOES.md: sem reserva por enquanto (saldo disponível = saldo físico); reserva depende do PDV (B7) |
| RN-CRT-EST-07 | Estoque negativo não pode ser aceito silenciosamente. Bloqueio ou aceitação com ocorrência está [A DEFINIR].                                                                                                                                      | Sim      | Feito    | PR B3.2 · mesma decisão de RN-EST-04/PA-15: bloqueia, e a justificativa exigida na exceção é a "ocorrência", fica na auditoria |
| PA-15         | Estoque negativo é permitido? — Sugestão do documento: não por padrão; integrar venda e criar ocorrência quando inevitável.                                                                                                                       | Sim      | Feito    | PR B3.2 · decisão em DECISOES.md; "venda" como movimento automático fica para o B7 |
| E-02          | Erro no documento: RN-EST-01 soma ajustes duas vezes (usar RN-CRT-EST-01)                                                                                                                                                                         |          | Feito    | PR B3.2 · implementação usa a fórmula de RN-CRT-EST-01 (soma simples das quantidades já assinadas), evitando o erro apontado |
| AUD-07        | Evento de auditoria: Movimentos de estoque, ajuste, perda, transferência, estorno e inventário (seção 8.1)                                                                                                                                        |          | Parcial  | PR B3.2 · movimento e estorno auditados; transferência (B3.5) e inventário (B3.6) ainda não existem |

## B3.3 — Lotes, validade, FEFO/FIFO e bloqueio

| ID            | Item                                                                                                                                                                    | Decisão? | Status   | Evidência |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | -------- | --------- |
| RF-LOT-01     | Registrar lote, fabricação, validade, quantidade, fornecedor e localização.                                                                                             |          | Parcial  | PR B3.3 · tabela `lots` (lote, validade, localização = endereço); quantidade via movimentos; fornecedor ainda é texto livre (`reference`), sem campo estruturado |
| RF-LOT-02     | Listar produtos vencendo em 30, 60 e 90 dias.                                                                                                                           |          | Parcial  | PR B3.3 · view `expiring_lots` com `days_until_expiry` pronta; falta uma tela dedicada de alerta agrupando por 30/60/90 dias |
| RF-LOT-03     | Sinalizar e bloquear lote vencido conforme política.                                                                                                                    |          | Feito    | PR B3.3 · `setLotStatus` (bloqueio manual) + bloqueio automático de lote vencido em saída, testado |
| RF-LOT-04     | Priorizar lote para saída conforme FEFO ou FIFO.                                                                                                                        |          | Feito    | PR B3.3 · `register_stock_movement` calcula o lote sugerido (vence antes/entrou antes) e exige justificativa pra fugir da ordem, testado |
| RF-LOT-06     | Permitir ação comercial sobre produto próximo do vencimento [A DEFINIR].                                                                                                | Sim      | Pendente | Sem módulo de preço/promoção nesta etapa (sistema de estoque, não de PDV) — fora do escopo até haver um módulo comercial |
| RF-LOT-07     | Rastrear lote do recebimento até saída, perda ou transferência.                                                                                                         |          | Parcial  | PR B3.3 · entrada/saída/perda com lote testado; transferência depende do B3.5 |
| RN-LOT-01     | Produtos com validade devem usar FEFO como política sugerida; adoção obrigatória [A DEFINIR].                                                                           | Sim      | Feito    | PR B3.3 · decisão em DECISOES.md, testado |
| RN-LOT-02     | Produtos sem validade podem usar FIFO; política definitiva [A DEFINIR].                                                                                                 | Sim      | Feito    | PR B3.3 · decisão em DECISOES.md: obrigatório, mesma regra do FEFO, testado |
| RN-LOT-03     | Alertas padrão solicitados são 90, 60 e 30 dias antes da validade.                                                                                                      |          | Parcial  | PR B3.3 · `days_until_expiry` calculado na view; falta a tela de alerta em si |
| RN-LOT-04     | A ação automática ao vencer - bloquear venda, bloquear movimentação ou somente alertar - está [A DEFINIR].                                                              | Sim      | Feito    | PR B3.3 · decisão em DECISOES.md (PA-19): bloqueia disponibilidade, testado |
| RN-LOT-06     | Alterar validade depois do recebimento exige justificativa e auditoria; perfis permitidos [A DEFINIR].                                                                  | Sim      | Feito    | PR B3.3 · decisão em DECISOES.md; `update_lot_expiry` exige justificativa (dono/gerente via RLS), testado |
| RN-CRT-VAL-01 | FEFO: para produto com validade, sugerir primeiro o lote que vence antes, desde que esteja liberado.                                                                    |          | Feito    | PR B3.3 · `order by expires_at asc nulls last`, ignora lotes bloqueados/vencidos, testado |
